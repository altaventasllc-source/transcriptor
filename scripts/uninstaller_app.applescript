on run
	set msg to "Se eliminaran:" & return & return & "  - Carpeta ~/Transcriptor (proyecto, modelos, uploads)" & return & "  - Transcriptor.app del Escritorio" & return & "  - Cache del modelo de IA (~1.5 GB)" & return & return & "NO se eliminaran (pueden usarlos otras apps):" & return & "  - Homebrew, Python, FFmpeg, Deno"
	...
	if button returned of theResponse is "Desinstalar" then
		try
			...
			display dialog "Transcriptor desinstalado correctamente." buttons {"OK"} default button "OK" with icon note with title "Desinstalar Transcriptor"
		on error errMsg
